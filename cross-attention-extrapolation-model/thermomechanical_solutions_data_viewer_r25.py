...
# =============================================
# FIXED SUNBURST VISUALIZER (globally unique labels)
# =============================================
def create_sunburst_chart(summaries, selected_field, colormap='Viridis', highlight_sim=None):
    """
    Robust sunburst chart using ONLY peak values (physical hierarchy).
    Hierarchy: All Simulations → Energy → Duration → Simulation
    Each simulation node gets value = peak(selected_field)
    Parent values are sums of children (branchvalues="total").
    
    CRITICAL FIX: Uses globally unique labels for duration nodes:
    dur_key = f"{energy_label}|{duration_label}" 
    This prevents collisions when the same duration appears under different energies.
    """
    labels = []      # display labels (may be cleaned)
    ids = []         # unique internal IDs (used for hierarchy)
    parents = []
    values = []
    colors = []
    ...
    energy_nodes = {}   # energy_label -> index in ids list
    duration_nodes = {} # dur_key -> index

    for s in summaries:
        sim_label = s['name']
        energy_label = f"{s['energy']:.1f} mJ"
        duration_label = f"{s['duration']:.1f} ns"
        dur_key = f"{energy_label}|{duration_label}"  # globally unique

        # Get peak value for selected field
        peak = s.get('field_stats', {}).get(selected_field, {}).get('max', [0])[0]
        peak = float(peak) if peak > 0 else 1e-3  # avoid zero

        # --- Energy node ---
        if energy_label not in energy_nodes:
            labels.append(energy_label)
            ids.append(energy_label)
            parents.append("root")
            values.append(0)
            colors.append("#ff7f0e")
            energy_nodes[energy_label] = len(ids) - 1
        energy_idx = energy_nodes[energy_label]

        # --- Duration node (using unique key as ID) ---
        if dur_key not in duration_nodes:
            # Display label: we can show just the duration value, but keep unique ID
            labels.append(duration_label)   # clean display
            ids.append(dur_key)             # unique ID for hierarchy
            parents.append(energy_label)    # parent is the energy node (by its unique ID)
            values.append(0)
            colors.append("#2ca02c")
            duration_nodes[dur_key] = len(ids) - 1
        dur_idx = duration_nodes[dur_key]

        # --- Simulation node ---
        sim_id = sim_label   # simulation names are unique
        labels.append(sim_label)
        ids.append(sim_id)
        parents.append(dur_key)    # parent is the duration's unique ID
        values.append(peak)
        ...
        # Accumulate values upward
        values[dur_idx] += peak
        values[energy_idx] += peak
        total_peak += peak

    # Update root value
    values[0] = total_peak

    # Optional: debug check for duplicate IDs
    # assert len(ids) == len(set(ids)), "Duplicate IDs detected!"

    # Color the leaves (simulation nodes) using the selected colormap based on peak value
    leaf_indices = [i for i, p in enumerate(parents) if p in duration_nodes]
    ...
    if leaf_values:
        ...
        for idx in leaf_indices:
            ...
            color_idx = int(norm_val * 100)
            colors[idx] = color_scale[color_idx]

    # Create figure using ids for unique identification
    fig = go.Figure(go.Sunburst(
        labels=labels,
        ids=ids,                # critical: ensures unique node identity
        parents=parents,
        values=values,
        branchvalues="total",
...
def render_data_viewer(selected_colormap):
    ...
    # Fix syntax error: iterate over fig.data
    ...
    # ================= IMPROVED SUNBURST SECTION (auto-update) =================
    st.markdown('<h2 class="sub-header">🌳 Comparative Sunburst – Peak Values</h2>', unsafe_allow_html=True)
    st.markdown("Hierarchy: **All Simulations → Energy → Duration → Simulation**  (radius = peak value of selected field)")
