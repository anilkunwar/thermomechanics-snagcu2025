# advanced_fea_attention_platform_fixed.py
...
class AdvancedVisualizer:
    """Comprehensive visualization components with IndexError fix"""
    ...
    def create_attention_network(attention_weights, source_simulations, top_k=10):
        """Create network graph showing top attention connections (AGGREGATED BY SIMULATION)"""
        if len(attention_weights) == 0 or len(source_simulations) == 0:
            fig = go.Figure()
            fig.update_layout(title="No attention data available", height=400)
            return fig

        # Reconstruct timestep counts per simulation
        timestep_counts = [len(s['timesteps']) for s in source_simulations]
        
        # Aggregate attention per simulation (sum over timesteps)
        sim_attention = []
        start_idx = 0
        for count in timestep_counts:
            end_idx = start_idx + count
            if end_idx <= len(attention_weights):
                sim_weight = np.sum(attention_weights[start_idx:end_idx])
                sim_attention.append(sim_weight)
            else:
                sim_attention.append(0.0)
            start_idx = end_idx

        sim_attention = np.array(sim_attention)
        
        # Get top-k simulations by aggregated attention
        valid_indices = np.where(sim_attention > 0)[0]
        if len(valid_indices) == 0:
            fig = go.Figure()
            fig.update_layout(title="No significant attention weights", height=400)
            return fig
            
        top_sim_indices = valid_indices[np.argsort(sim_attention[valid_indices])[-top_k:][::-1]]
        top_sim_weights = sim_attention[top_sim_indices]
        top_sims = [source_simulations[i] for i in top_sim_indices]

        # Create network
        G = nx.Graph()
        G.add_node("Query", size=50, color='red')

        for i, (sim_idx, weight) in enumerate(zip(top_sim_indices, top_sim_weights)):
            node_id = f"Sim{i}"
            G.add_node(node_id, 
                      size=30 * weight / (np.max(top_sim_weights) + 1e-8),
                      color='blue',
                      energy=source_simulations[sim_idx]['energy'],
                      duration=source_simulations[sim_idx]['duration'],
                      weight=weight)
        ...
        for node in G.nodes():
            ...
            if node == "Query":
                node_text.append("Query<br>(E, τ, t)")
                node_size.append(50)
                node_color.append('red')
            else:
                idx = int(node.replace("Sim", ""))
                sim = top_sims[idx]
                weight = top_sim_weights[idx]
                node_text.append(f"E: {sim['energy']}mJ<br>τ: {sim['duration']}ns<br>Attention: {weight:.3f}")
                node_size.append(30 * weight / (np.max(top_sim_weights) + 1e-8))
        ...
        fig.update_layout(
            title=f"Top {len(top_sims)} Most Relevant Simulations",
            showlegend=False,
            hovermode='closest',
