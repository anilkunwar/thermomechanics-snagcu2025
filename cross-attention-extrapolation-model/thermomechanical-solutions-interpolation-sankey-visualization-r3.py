...
class EnhancedVisualizer:
    ...
    def create_stdgpa_sankey(self, results, energy_query, duration_query, selected_time,
        ...
        labels.extend(["Physics Attention", "(E, τ, t) Gating", "Temporal Similarity", "ST-DGPA Final Weight"])
...
def render_stdgpa_attention_visualization(results, energy_query, duration_query, time_points):
    """Render ST-DGPA analysis with integrated Sankey diagram."""
    if not results.get('physics_attention_maps') or len(results['physics_attention_maps'][0]) == 0: st.info("No ST-DGPA attention data."); return
    ...
    sankey_fig = st.session_state.visualizer.create_stdgpa_sankey(
        ...
        physics_attention=physics_attention, ett_gating=ett_gating, temporal_sim=temporal_sim, top_k=10
    )
    ...
    if len(final_weights) > 0:
        comparison_data = []
        for i in range(min(10, len(final_weights))):
            comparison_data.append({'Source': f"Source {i+1}", 'Physics Attn': f"{physics_attention[i]:.4f}", '(E,τ,t) Gating': f"{ett_gating[i]:.4f}", 'ST-DGPA Final': f"{final_weights[i]:.4f}"})
