...
def render_interpolation_extrapolation():
        ...
        with st.spinner("Running multi-head attention prediction with spatial locality regulation..."):
            ...
            if results and 'field_predictions' in results and results['field_predictions']:
                ...
                prediction_id = hashlib.md5(
                    f"{energy_query}_{duration_query}_{sigma_param}_{spatial_weight}".encode()
                ).hexdigest()[:8]
    ...
    # If results already exist from previous run, show them
        ...
        st.markdown(f"""
        <div class="info-box">
        <h3>📊 Previous Prediction Results Available</h3>
        <p>Showing results from previous prediction run.</p>
