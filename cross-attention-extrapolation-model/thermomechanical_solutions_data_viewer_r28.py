...
# =============================================
# CONSTANTS (Extended for better hierarchy control)
# =============================================
EXTENDED_COLORMAPS = [
...
# =============================================
# ENHANCED SUNBURST VISUALIZER (MULTI-COLORMAP)
# =============================================
def create_sunburst_chart(summaries, selected_field, 
    ...
    """
    Robust Sunburst Visualizer with independent colormaps for each hierarchy level.
    
    Fixes & Features:
    - Strict ID mapping for parents to avoid Plotly rendering bugs.
    - Explicit bottom-up value aggregation for `branchvalues="total"`.
    - Clean labels with peak values moved to hover.
    - 4 independent colorbars for Root, Energy, Tau, and Field.
    """
    ...
    parents.append("") 
    values.append(0)   
    colors.append("#E0E0E0") # Default placeholder, overwritten below
    levels.append(0)
    root_idx = 0
    ...
    # 3. Independent Color Map Sampling per Hierarchy
    ...
    # 8. Layout & Independent Colorbars
    fig.update_layout(
        margin=dict(t=40, l=10, r=240, b=10),
        paper_bgcolor="rgba(0,0,0,0)"
    )

    # Add distinct colorbars using dummy traces for each attribute
    colorbar_data = [
        ("FEM Root", cmap_root, 0, 1, 0.96),
        ("Energy (mJ)", cmap_energy, e_min, e_max, 0.74),
        ("Pulse Width (ns)", cmap_tau, d_min, d_max, 0.51),
        (f"Peak {selected_field}", cmap_field, p_min, p_max, 0.28)
    ]

    for title, cmap, cmin, cmax, y_pos in colorbar_data:
...
def render_data_viewer(selected_colormap):
    ...
    # ✅ Chart renders reactively with independent colormaps
