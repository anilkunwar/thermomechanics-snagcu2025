...
# -*- coding: utf-8 -*-
"""
ENHANCED FEA LASER SIMULATION PLATFORM WITH ST-DGPA & RADAR CHART
====================================================================
- Full ST-DGPA interpolation/extrapolation
- Advanced 3D field rendering with caching
- Physics-aware Sankey diagram for weight decomposition
- 🔥 NEW: Robust polar radar chart for source simulation comparison
- Dynamic data loading with comprehensive error handling
"""
...
class EnhancedVisualizer:
    ...
    def create_stdgpa_sankey(self, results, energy_query, duration_query, selected_time,
        ...
        labels.extend(["⚛️ Physics Attention", "⚙️ (E,τ,t) Gating", "⏱️ Temporal Sim.", "🎯 ST-DGPA Final"])
    ...
    # =============================================
    # NEW: ROBUST POLAR RADAR CHART
    # =============================================
    def create_enhanced_polar_radar_chart(self, df, field_type, timestep, 
                                          normalize_by_max=True, max_reference_value=None,
                                          radial_axis_range=None, width=800, height=600,
                                          source_symbol='circle', target_symbol='star-diamond',
                                          c_scale='Viridis', title_field="Peak Value"):
        """
        Create a polar radar chart comparing source simulations based on energy (angle) and duration (radius),
        with marker size/color representing a field's peak value (e.g., temperature, stress).
        
        Enhanced with robust validation, normalization, jitter, and informative warnings.
        """
        # === VALIDATE INPUTS ===
        if df.empty:
            st.error("❌ No source simulation data available for radar chart")
            return go.Figure().update_layout(title="No Data", width=width, height=height)
        
        # Extract columns with safety
        sim_names = df['Name'].tolist()
        ...
        valid_mask = (np.isfinite(energies) & np.isfinite(durations) & 
                      np.isfinite(peak_values) & (durations >= 0))
        if not np.any(valid_mask):
            st.error("❌ All data rows contain invalid values (NaN/Inf/negative). Cannot plot.")
            return go.Figure().update_layout(title="Invalid Data", width=width, height=height)
        ...
        st.caption(f"✅ Plotting {len(energies)} valid source simulations (field: {field_type}, timestep: {timestep})")
        
        # === SAFE NORMALIZATION ===
        def safe_normalize(values, reference_max=None):
            ...
            if c_max - c_min < 1e-9:
                return np.ones_like(clean) * 0.5  # Mid-range
            return (clean - c_min) / (c_max - c_min)
        
        norm_peak = safe_normalize(peak_values, max_reference_value if normalize_by_max else None)
        
        # === ROBUST ANGLE MAPPING (Energy -> Angle) ===
        e_min, e_max = np.min(energies), np.max(energies)
        e_range = e_max - e_min
        if e_range < 1e-6:
            e_range = 1.0
            e_min = e_min - 0.5  # Center single value
        angles_deg = np.degrees(2 * np.pi * (energies - e_min) / e_range)
        
        # Add jitter for overlapping points
        unique_angles = np.unique(np.round(angles_deg, 1))
        if len(unique_angles) < len(angles_deg) * 0.9:
        ...
        # Add source simulations trace
        fig.add_trace(go.Scatterpolar(
            r=durations,
            theta=angles_deg,
            mode='markers',
            marker=dict(
                size=10 + 20 * norm_peak,   # Size range [10,30]
                color=norm_peak,
                colorscale=c_scale,
                colorbar=dict(title=title_field, thickness=20, x=1.02) if normalize_by_max else None,
                line=dict(width=2, color='white'),
                symbol=source_symbol
            ),
            text=[f"<b>{name}</b><br>E: {e:.1f} mJ<br>τ: {d:.1f} ns<br>Peak: {p:.3f}"
        ...
        # Optionally add target query point (if provided as separate arguments)
        # (Not used in this standalone method, but can be extended)
        ...
        fig.update_layout(
            polar=dict(
                radialaxis=dict(
                    title="Duration (ns)",
                    range=radial_axis_range if radial_axis_range else [0, max(durations) * 1.05],
                ...
                angularaxis=dict(
                    ...
                    gridcolor='lightgray',
                    direction="clockwise",
                    rotation=90  # Put 0° at top
                ),
                bgcolor='rgba(245,245,245,0.8)'
            ),
            title=dict(
                text=f"<b>Source Simulation Comparison</b><br>{field_type} Peak Value at t={timestep}",
                font=dict(size=20),
                x=0.5
            ),
            width=width,
...
def render_stdgpa_attention_visualization(results, energy_query, duration_query, time_points):
    ...
    if not results.get('physics_attention_maps') or len(results['physics_attention_maps'][0]) == 0:
        ...
        return
    st.markdown('<h4 class="sub-header">🧠 ST-DGPA Weight Decomposition (Sankey)</h4>', unsafe_allow_html=True)
...
# =============================================
# NEW: RADAR CHART COMPARISON MODE
# =============================================
def render_radar_comparison():
    st.markdown('<h2 class="sub-header">📡 Comparative Radar Analysis</h2>', unsafe_allow_html=True)
    if not st.session_state.data_loaded:
        st.markdown('<div class="warning-box"><h3>⚠️ No Data Loaded</h3><p>Please load simulations first using the sidebar button.</p></div>', unsafe_allow_html=True)
        return
    
    # Gather all available fields across simulations
    all_fields = set()
    for s in st.session_state.summaries:
        all_fields.update(s['field_stats'].keys())
    if not all_fields:
        st.warning("No field data available in summaries.")
        return
    
    # Side-by-side controls
    col1, col2 = st.columns(2)
    with col1:
        field_type = st.selectbox("Select Field to Compare", sorted(all_fields), key="radar_field")
    with col2:
        # Determine max timestep across simulations
        max_ts = max([len(s['timesteps']) for s in st.session_state.summaries]) if st.session_state.summaries else 1
        timestep = st.number_input("Timestep (1-indexed)", min_value=1, max_value=max_ts, value=1, step=1, key="radar_ts")
    
    # Build DataFrame of source simulations
    rows = []
    for s in st.session_state.summaries:
        ts_idx = timestep - 1
        if ts_idx >= len(s['timesteps']):
            st.warning(f"⚠️ {s['name']}: timestep {timestep} not available (max {len(s['timesteps'])})")
            continue
        ...
        if ts_idx >= len(peak_list):
            st.warning(f"⚠️ {s['name']}: insufficient data for timestep {timestep}")
            continue
        peak_val = peak_list[ts_idx]
    ...
    if not rows:
        st.error(f"❌ No valid data for field='{field_type}' at timestep={timestep}")
    ...
    # Radar chart options
    with st.expander("🎨 Radar Chart Options"):
        normalize = st.checkbox("Normalize marker size/color by global maximum", value=True)
        if normalize:
            # Compute global max across all simulations for this field at this timestep
            all_peaks = [row['Peak_Value'] for row in rows]
            global_max = max(all_peaks) if all_peaks else 1.0
            max_ref = global_max
        else:
            max_ref = None
        radial_min = st.number_input("Radial axis min (ns)", value=0.0, step=1.0)
        radial_max = st.number_input("Radial axis max (ns)", value=float(df['Duration'].max() * 1.1), step=1.0)
    
    # Create radar chart
    fig = st.session_state.visualizer.create_enhanced_polar_radar_chart(
        df=df,
        field_type=field_type,
        timestep=timestep,
        normalize_by_max=normalize,
        max_reference_value=max_ref,
        radial_axis_range=(radial_min, radial_max),
        width=800,
        height=600,
        source_symbol='circle',
        title_field=f"{field_type} Peak Value"
    )
...
def main():
    ...
    with st.sidebar:
        ...
        app_mode = st.radio("Select Mode", ["Data Viewer", "Interpolation/Extrapolation", "Radar Comparison"], key="nav_mode")
        st.markdown("---")
        if st.button("🔄 Load All Simulations", type="primary", use_container_width=True):
            with st.spinner("Loading simulations..."):
                ...
                if sims and summaries:
                    ...
                    st.error("No simulations found. Check folder path.")
        st.markdown("---")
    ...
    elif app_mode == "Radar Comparison":
