...
def create_sunburst_chart(
    ...
    for (e, d), d_idx in d_indices.items():
        if e in tree and d in tree[e]:
            d_sum = sum(peak for (peak, _) in tree[e][d].values())
            values[d_idx] = d_sum
    ...
    # Build font sizes list matching each node's level
    node_font_sizes = []
    for lvl in levels:
        if lvl == 0:
            node_font_sizes.append(font_sizes.get('root', 16))
        elif lvl == 1:
            node_font_sizes.append(font_sizes.get('energy', 14))
        elif lvl == 2:
            node_font_sizes.append(font_sizes.get('duration', 12))
        else:
            node_font_sizes.append(font_sizes.get('name', 10))
    ...
    fig = go.Figure(go.Sunburst(
        ...
        textfont=dict(size=node_font_sizes, family="sans-serif"),
    ...
    if colorbar_side == 'split':
        ...
        titles = ["FEM Root", "Energy (mJ)", "Pulse Width (ns)", f"Peak {selected_field}"]
