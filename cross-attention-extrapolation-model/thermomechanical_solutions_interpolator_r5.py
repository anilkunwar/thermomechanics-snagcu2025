...
class FEAVisualizationPlatform:
            ...
            if st.button("🔄 Load All Simulations", type="primary", use_container_width=True):
                with st.spinner("Loading simulation data..."):
                    ...
                    if simulations and summaries:
                        ...
                        # Note: The actual extrapolator initialization would go here
    ...
    def render_prediction_demo(self, energy, duration, time, field, ref_sim, viz_type):
        ...
        # Create spatial pattern based on query parameters
        centroid = np.mean(mesh_data.points, axis=0)
        distances = np.linalg.norm(mesh_data.points - centroid, axis=1)
        distances_norm = distances / np.max(distances)
        
        # Simulate laser heating pattern
        pattern = np.exp(-distances_norm ** 2 / 0.2)
        
        # Scale by energy and time
        scaled_pattern = pattern * energy * (1 - np.exp(-time / duration))
        
        # Add noise for realism
        noise = np.random.normal(0, 0.1, n_points)
        predicted_values = scaled_pattern + noise
        
        # Ensure non-negative
        predicted_values = np.maximum(predicted_values, 0)
        
        # Display prediction
        st.success(f"✅ Prediction generated for E={energy:.1f}mJ, τ={duration:.1f}ns, t={time:.1f}ns")
        ...
        if viz_type == "3D Mesh":
            ...
            for t in time_points:
                pattern_t = pattern * energy * (1 - np.exp(-t / duration))
                field_evolution.append(pattern_t + np.random.normal(0, 0.05, n_points))
