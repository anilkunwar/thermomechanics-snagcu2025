    ...
    def create_radar_chart(summaries, simulation_names, target_sim=None):
        ...
        for sim_name in simulation_names:
            ...
            fig.add_trace(go.Scatterpolar(
                ...
                fillcolor=f'rgba(255,0,0,0.3)' if target_sim and sim_name == target_sim else None,
                opacity=0.8
            ))
...
def main():
    ...
    with st.sidebar:
        ...
        app_mode = st.radio(
            "Select Mode",
            ["Data Viewer", "3D Visualization", "Interpolation/Extrapolation", "Comparative Analysis"],
            index=["Data Viewer", "3D Visualization", "Interpolation/Extrapolation", "Comparative Analysis"].index(
...
def render_3d_visualization():
    """Render enhanced 3D visualization with interpolation"""
    ...
    if 'points' in sim and 'fields' in sim and field in sim['fields']:
        ...
        st.markdown('<h3 class="sub-header">📊 Field Analysis</h3>', unsafe_allow_html=True)
        ...
        with st.expander("📈 Detailed Field Analysis", expanded=False):
            tab1, tab2, tab3 = st.tabs(["Distribution", "Spatial Stats", "Export"])
            ...
            with tab2:
                ...
                if len(pts) > 0:
                    col1, col2 = st.columns(2)
                    with col1:
                        st.write("**Spatial Extents:**")
                        st.write(f"X: {np.min(pts[:, 0]):.3f} to {np.max(pts[:, 0]):.3f}")
                        st.write(f"Y: {np.min(pts[:, 1]):.3f} to {np.max(pts[:, 1]):.3f}")
                        st.write(f"Z: {np.min(pts[:, 2]):.3f} to {np.max(pts[:, 2]):.3f}")
                    
                    with col2:
                        st.write("**Value- Position Correlation:**")
                        if len(values) == len(pts):
                            correlations = []
                            for i in range(3):
                                corr = np.corrcoef(pts[:, i], values)[0, 1]
                                correlations.append(corr)
                            
                            st.write(f"X correlation: {correlations[0]:.3f}")
                            st.write(f"Y correlation: {correlations[1]:.3f}")
                            st.write(f"Z correlation: {correlations[2]:.3f}")
            
            with tab3:
                # Export options
                st.write("**Export Visualization Data:**")
                ...
                with col1:
                    if st.button("📥 Export as CSV", use_container_width=True):
                        ...
                        export_data = np.column_stack([pts, values])
                        df = pd.DataFrame(export_data, columns=['X', 'Y', 'Z', 'Value'])
                ...
                with col2:
                    if st.button("📊 Export Statistics", use_container_width=True):
                        ...
                        st.download_button(
                            ...
                            file_name=f"{sim_name}_{field}_t{timestep}_stats.json",
                ...
                with col3:
                    if st.button("🖼️ Export as Image", use_container_width=True):
                        ...
                        st.download_button(
                            label="Download PNG",
                            data=img_bytes,
                            file_name=f"{sim_name}_{field}_t{timestep}.png",
                            mime="image/png"
                        )
