...
# =============================================
# 🔧 ENUM: SANKEY SOURCE SELECTION STRATEGIES
# =============================================
class SankeySourceStrategy:
    """Enumeration of source selection strategies for Sankey diagram."""
    
    TOP_BY_FINAL_WEIGHT = "Top-n by ST-DGPA Final Weight"
    BOTTOM_BY_FINAL_WEIGHT = "Bottom-n by ST-DGPA Final Weight"
    RANDOM = "Random-n (Uniform)"
    TOP_BY_PHYSICS = "Top-n by Physics Attention"
    TOP_BY_GATING = "Top-n by (E,τ,t) Gating"
    TOP_BY_TEMPORAL = "Top-n by Temporal Similarity"
    
    ALL_STRATEGIES = [
        TOP_BY_FINAL_WEIGHT,
        BOTTOM_BY_FINAL_WEIGHT,
        RANDOM,
        TOP_BY_PHYSICS,
        TOP_BY_GATING,
        TOP_BY_TEMPORAL
    ...
    DESCRIPTIONS = {
        TOP_BY_FINAL_WEIGHT: "Select sources with highest final ST-DGPA attention weights (most influential contributors)",
        BOTTOM_BY_FINAL_WEIGHT: "Select sources with lowest final ST-DGPA weights (least influential, for sensitivity analysis)",
        RANDOM: "Random uniform sampling from all available sources (for statistical exploration)",
        TOP_BY_PHYSICS: "Select sources with highest pure physics-based attention (before gating)",
        TOP_BY_GATING: "Select sources closest in (Energy, Duration, Time) parameter space to query",
        TOP_BY_TEMPORAL: "Select sources most similar in temporal evolution to query time point"
    }
    
    @staticmethod
    def get_strategy_key(strategy_name: str) -> str:
        """Convert strategy name to internal key for session state."""
        mapping = {
            SankeySourceStrategy.TOP_BY_FINAL_WEIGHT: "top_final",
            SankeySourceStrategy.BOTTOM_BY_FINAL_WEIGHT: "bottom_final",
            SankeySourceStrategy.RANDOM: "random",
            SankeySourceStrategy.TOP_BY_PHYSICS: "top_physics",
            SankeySourceStrategy.TOP_BY_GATING: "top_gating",
            SankeySourceStrategy.TOP_BY_TEMPORAL: "top_temporal"
        }
        return mapping.get(strategy_name, "top_final")
...
class SankeyCustomizationManager:
    ...
    def initialize_session_state():
        ...
        if 'sankey_random_seed' not in st.session_state:
            st.session_state.sankey_random_seed = 42
    ...
    def export_customizations() -> str:
        ...
        config = {
            ...
            'source_strategy': st.session_state.sankey_source_strategy,
            'random_seed': st.session_state.sankey_random_seed,
    ...
    def import_customizations(json_str: str) -> bool:
        ...
        try:
            ...
            if 'source_strategy' in config:
                st.session_state.sankey_source_strategy = config['source_strategy']
            if 'random_seed' in config:
                st.session_state.sankey_random_seed = config['random_seed']
            return True
...
# =============================================
# 🔧 SANKEY SOURCE SELECTOR UTILITY
# =============================================
class SankeySourceSelector:
    """Utility class for selecting source indices based on different strategies."""
    
    @staticmethod
    def select_sources(
        attention_weights: np.ndarray,
        physics_attention: np.ndarray,
        ett_gating: np.ndarray,
        temporal_sim: Optional[np.ndarray],
        n_select: int,
        strategy: str,
        random_seed: int = 42
    ) -> np.ndarray:
    ...
    @staticmethod
    def get_strategy_badge_html(strategy: str) -> str:
        """Generate HTML badge for strategy indicator."""
        color = SankeySourceStrategy.get_strategy_color(strategy)
        description = SankeySourceStrategy.DESCRIPTIONS.get(strategy, "")
        return f'''
        <span style="
...
class EnhancedVisualizer:
    ...
    def create_stdgpa_sankey(self, results, energy_query, duration_query, selected_time,
        ...
        fig.update_layout(
            title=dict(
                text=f"<b>ST-DGPA Weight Decomposition</b><br>Query: E={energy_query:.1f} mJ, τ={duration_query:.1f} ns, t={selected_time:.1f} ns<br>{strategy_badge}<br>(Showing {top_k} of {total_sources} sources)",
                font=dict(size=20, family=font_family, color="#2c3e50"),
                x=0.5, y=0.95
            ),
...
def render_stdgpa_attention_visualization(results, energy_query, duration_query, time_points):
    ...
    st.markdown('<h4 class="sub-header">🧠 ST-DGPA Weight Decomposition (Advanced Sankey)</h4>', unsafe_allow_html=True)
    
    col_timestep, col_topk, col_strategy = st.columns([2, 1, 2])
    with col_timestep:
    ...
    with col_topk:
        ...
        top_k = st.slider(
            ...
            min_value=3, max_value=max_topk, value=min(12, max_topk),
            help="Number of source entries to show in Sankey diagram",
            key="stdgpa_topk"
        )
    with col_strategy:
        source_strategy = st.selectbox(
            ...
            index=SankeySourceStrategy.ALL_STRATEGIES.index(
                st.session_state.get('sankey_source_strategy', SankeySourceStrategy.TOP_BY_FINAL_WEIGHT)
            ),
        ...
        if source_strategy == SankeySourceStrategy.RANDOM:
            random_seed = st.number_input(
                "🎲 Random Seed",
                min_value=0, max_value=999999, value=st.session_state.get('sankey_random_seed', 42),
                key="sankey_random_seed_input",
                help="Seed for reproducible random sampling"
            )
            st.session_state.sankey_random_seed = random_seed
    ...
    if len(final_weights) > 0:
        ...
        for i, idx in enumerate(selected_indices):
            ...
            comparison_data.append({
                ...
                'Energy E (mJ)': f"{meta['energy']:.1f}",
                'Duration τ (ns)': f"{meta['duration']:.1f}",
                'Time t (ns)': f"{meta['time']:.1f}",
                'Physics Attn': f"{physics_attention[idx]:.4f}",
                '(E,τ,t) Gating': f"{ett_gating[idx]:.4f}",
                'Temporal Sim': f"{temporal_sim[idx]:.4f}" if temporal_sim is not None else "N/A",
        ...
        strategy_explanation = {
            ...
            SankeySourceStrategy.RANDOM: f"Random uniform sampling (seed={st.session_state.sankey_random_seed}) for statistical exploration",
            SankeySourceStrategy.TOP_BY_PHYSICS: "Sources with highest pure physics-based attention scores",
            SankeySourceStrategy.TOP_BY_GATING: "Sources closest in (Energy, Duration, Time) parameter space to query",
            SankeySourceStrategy.TOP_BY_TEMPORAL: "Sources most similar in temporal evolution to query time point"
        }
        st.caption(f"💡 {strategy_explanation.get(source_strategy, '')}")
        st.markdown(f"**Total sources available:** {len(final_weights)} | **Displayed:** {top_k} via `{source_strategy}`")
...
def render_3d_analysis(results, time_points, energy_query, duration_query):
    ...
    fig_temp.update_layout(
        ...
        scene=dict(xaxis_title="Energy E (mJ)", yaxis_title="Duration τ (ns)", zaxis_title="Max Temp"), 
        height=500
    )
...
def main():
    st.set_page_config(
        page_title="Enhanced FEA Laser Platform with Advanced Sankey", 
        layout="wide", 
