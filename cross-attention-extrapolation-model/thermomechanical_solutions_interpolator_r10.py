        ...
        for folder_idx, folder in enumerate(folders):
            ...
            try:
                ...
                if load_full_mesh:
                    ...
                    # Find triangles - Enhanced to handle multiple cell types
                    triangles = None
                    ...
                    # Load remaining timesteps with error handling
...
class EnhancedPhysicsInformedAttentionExtrapolator:
    ...
    def interpolate_full_field(self, field_name, attention_weights, source_metadata, simulations):
        ...
        first_sim = next(iter(simulations.values()), None)
        if first_sim is None or 'fields' not in first_sim or field_name not in first_sim['fields']:
            st.warning(f"Field '{field_name}' not found in source simulations.")
            return None
        ...
        for idx, weight in enumerate(attention_weights):
            ...
            if sim_name in simulations:
                ...
                if 'fields' in sim and field_name in sim['fields']:
                    ...
                    if source_field.shape != interpolated_field.shape:
                        st.warning(f"Shape mismatch for {field_name} in {sim_name}. Skipping.")
                        continue
                    interpolated_field += weight * source_field
                    total_weight += weight
                    used_sources += 1
       
        if total_weight > 0 and used_sources > 0:
            interpolated_field /= total_weight
            st.info(f"Interpolated using {used_sources} sources.")
        else:
            st.warning(f"No valid sources for interpolating '{field_name}'.")
            return None
...
def main():
    ...
    # Initialize session state with additional keys for field handling
    if 'data_loader' not in st.session_state:
        ...
        st.session_state.previous_field = None  # To track field changes for cache clearing
...
# Update render_data_viewer to handle field changes without crash
...
└── q2p0mJ-delta1p0ns/ # Energy: 2.0 mJ, Duration: 1.0 ns
    ...
    sim = simulations.get(sim_name, None)  # Safe get to avoid key error
    if sim is None:
        st.error(f"Simulation {sim_name} not found. Try reloading.")
        return
    ...
    # Detect field change and clear relevant cache if needed
    if 'previous_field' in st.session_state and st.session_state.previous_field != field:
        st.cache_data.clear()  # Clear cache on field change to prevent crashes
        st.session_state.previous_field = field
   
    # Main 3D visualization with enhanced error handling
    try:
        if 'points' in sim and 'fields' in sim and field in sim['fields']:
            pts = sim['points']
            kind, dim = sim['field_info'][field]
            raw = sim['fields'][field][timestep]
           
            if kind == "scalar":
                values = np.nan_to_num(raw)
                label = field
            else:
            ...
            if sim.get('triangles') is not None and len(sim['triangles']) > 0:
                ...
                valid_mask = np.all((tri >= 0) & (tri < len(pts)), axis=1)
                valid_triangles = tri[valid_mask]
        ...
        st.error(f"Error rendering visualization for '{field}': {str(e)}")
        st.info("Try selecting a different field or reloading the simulations.")

# (Rest of the code remains the same, with similar try-except in render_3d_interpolation_visualization for robustness)
