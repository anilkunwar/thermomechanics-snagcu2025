...
# =============================================
# SUNBURST CHART HELPER FUNCTIONS (Replicated from KB)
# =============================================
def create_sunburst_chart(summaries, selected_field='temperature', highlight_sim=None):
    ...
    Hierarchy: All Simulations → Energy (E) → Simulation → Field Peak (leaf)
    (Fixed loop-scoping bug from original KB code so all energy groups are processed)
    """
    labels = []
    parents = []
    values = []
    ...
    # Add energy level nodes and their children
    for energy_key, energy_sims in energy_groups.items():
        ...
        # Add simulation nodes for each energy group
...
def render_data_viewer():
    ...
    if trace_data is None:
        trace_data = go.Scatter3d(
            x=pts[:, 0], y=pts[:, 1], z=pts[:, 2],
            mode='markers',
            marker=dict(size=point_size, color=values, colorscale="Viridis", cmin=cmin, cmax=cmax, opacity=opacity),
    ...
    The hierarchy is: **All Simulations → Energy (E) → Simulation → Field Peak**.
    """)
    ...
    if len(available_fields) < 1:
        st.warning("Need at least one field to display the sunburst chart.")
    else:
        ...
        if st.button("Generate Sunburst Charts", type="primary", use_container_width=True):
            with st.spinner("Building sunburst charts..."):
                ...
                fig1 = create_sunburst_chart(summaries, field1, hl)
                fig2 = create_sunburst_chart(summaries, field2, hl)
