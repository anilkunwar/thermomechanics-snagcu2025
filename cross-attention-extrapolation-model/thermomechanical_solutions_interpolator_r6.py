...
# =============================================
# PHYSICS-INFORMED EXTRAPOLATOR
# =============================================
class PhysicsInformedExtrapolator:
    """Extrapolator using attention-based mechanism for field statistics"""
    
    def __init__(self):
        self.fitted = False
        self.source_points = None  # (N, 3) array: energy, duration, timestep_idx
        self.source_stats = {}  # field_name -> (N, num_stats) array
        self.source_metadata = []  # list of dicts
        self.scaler = StandardScaler()
    
    def fit(self, summaries, common_fields):
        sources = []
        metadata = []
        ...
        for summary in summaries:
            ...
            e = summary['energy']
            d = summary['duration']
            n_times = len(summary['timesteps'])
            
            for t in range(n_times):
                source = [e, d, t]  # t is 0-based index
                ...
                metadata.append({
                    ...
                    'duration': d,
                    'timestep_idx': t,
                    'time': summary['timesteps'][t]  # 1-based
                })
        
        self.source_points = np.array(sources)
        self.source_points = self.scaler.fit_transform(self.source_points)
        self.source_metadata = metadata
        
        # Collect stats for each field
        stat_keys = ['min', 'max', 'mean', 'std', 'q25', 'q50', 'q75', 'skew', 'kurtosis']
        
        for field in common_fields:
            stats_list = []
            for summary in summaries:
                field_stats = summary['field_stats'][field]
                for t in range(len(summary['timesteps'])):
                    stat_vec = [field_stats[key][t] for key in stat_keys]
                    stats_list.append(stat_vec)
            self.source_stats[field] = np.array(stats_list)
        
        self.fitted = True
        print(f"Fitted extrapolator with {len(self.source_points)} source points")
    
    def predict_field_statistics(self, energy_query, duration_query, time_query):
        if not self.fitted:
            return None
        
        # time_query is 1-based, convert to 0-based index
        query_idx = time_query - 1 if time_query > 0 else 0
        query_point = np.array([[energy_query, duration_query, query_idx]])
        query_point = self.scaler.transform(query_point)
        
        # Compute distances
        dists = cdist(query_point, self.source_points, metric='euclidean')[0]
        
        # Attention weights
        sigma = np.mean(dists) / 2 if np.mean(dists) > 0 else 1.0
        if sigma == 0:
            sigma = 1.0
        att_scores = np.exp(-dists / sigma)
        att_weights = att_scores / np.sum(att_scores) if np.sum(att_scores) > 0 else np.zeros_like(att_scores)
        
        # Predict stats for each field
        field_predictions = {}
        stat_keys = ['min', 'max', 'mean', 'std', 'q25', 'q50', 'q75', 'skew', 'kurtosis']
        
        for field, source_stats in self.source_stats.items():
            pred_stats = np.dot(att_weights, source_stats)
            field_predictions[field] = dict(zip(stat_keys, pred_stats))
...
class GeometricalFieldPredictor:
    ...
    def predict_field_on_mesh(self, energy_query, duration_query, time_query, reference_mesh, field_name):
        ...
        if field_name.lower() in ['temperature', 'stress', 'displacement']:
...
class FEAVisualizationPlatform:
    ...
    def render_sidebar(self):
        ...
        with st.sidebar:
            ...
            if st.button("🔄 Load All Simulations", type="primary", use_container_width=True):
                with st.spinner("Loading simulation data..."):
                    ...
                    if simulations and summaries:
                        ...
                        # Initialize extrapolator and predictor
                        st.session_state.extrapolator = PhysicsInformedExtrapolator()
                        st.session_state.extrapolator.fit(summaries, self.data_loader.common_fields)
                        st.session_state.geom_predictor = GeometricalFieldPredictor(st.session_state.extrapolator, self.data_loader)
            ...
            if st.session_state.get('data_loaded', False):
                ...
                with st.expander("Mesh Display Options"):
                ...
                with st.expander("Field Display Options"):
                    st.checkbox("Show Field Statistics", value=True, key="show_field_stats")
                    st.checkbox("Show Cross-Sections", value=False, key="show_cross_sections")
    ...
    def render_interpolation_extrapolation(self):
        ...
        if 'extrapolator' not in st.session_state or not st.session_state.extrapolator.fitted:
            st.warning("⚠️ Extrapolator not fitted. Please load simulations first.")
            return
        ...
        if st.button("🚀 Generate Prediction", type="primary", use_container_width=True):
            ...
            with st.spinner("Generating physics-informed prediction..."):
                reference_mesh = st.session_state.simulations[reference_sim]['mesh_data']
                self.render_prediction(energy_query, duration_query, time_query,
                                      selected_field, reference_mesh, viz_type)
   
    def render_prediction(self, energy, duration, time, field, reference_mesh, viz_type):
        """Render actual prediction using loaded data"""
        if not st.session_state.data_loaded:
            return
       
        # Get prediction
        pred = st.session_state.geom_predictor.predict_field_on_mesh(energy, duration, time, reference_mesh, field)
        ...
        st.success(f"✅ Prediction generated for E={energy:.1f}mJ, τ={duration:.1f}ns, t={time} (Confidence: {confidence:.2%})")
        ...
        if viz_type == "3D Mesh":
            ...
            time_points = np.arange(1, time + 1, max(1, time // 10))
            field_evolution, confidences = st.session_state.geom_predictor.predict_field_evolution(
                energy, duration, time_points, reference_mesh, field
            )
        ...
        with col1:
            st.metric("Mean", f"{np.mean(predicted_values):.3f}")
        with col2:
            st.metric("Maximum", f"{np.max(predicted_values):.3f}")
        with col3:
    ...
    def render_field_evolution_comparison(self, selected_sims, selected_field):
        ...
        for sim_name in selected_sims:
            ...
            if selected_field in sim['field_info']:
                mesh_data = sim['mesh_data']
    ...
    def render_geometry_comparison(self, selected_sims, selected_field):
        ...
        for idx, sim_name in enumerate(selected_sims[:2]): # Limit to 2 for side-by-side
            sim = st.session_state.simulations[sim_name]
