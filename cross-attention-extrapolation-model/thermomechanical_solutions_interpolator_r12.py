...
class EnhancedPhysicsInformedAttentionExtrapolator:
    ...
    def interpolate_full_field(self, field_name, attention_weights, source_metadata, simulations, 
        ...
        """Compute interpolated full field using attention weights with caching support.
        ...
        Args:
            ...
            cache_key (str, optional): Key for caching. If None, a default key is created.
        ...
        if cache_key is None:
            weights_hash = hash(str(attention_weights[:10]))  # Just use first few weights for hash
            cache_key = f"{field_name}_{weights_hash}"
        
        # Check if we have a cached result
        if use_cache and 'interpolated_fields_cache' in st.session_state:
            cache = st.session_state.interpolated_fields_cache
        ...
        if len(field_shape) == 1:  # Scalar field
        ...
        # Cache the result
        if use_cache:
            if 'interpolated_fields_cache' not in st.session_state:
                st.session_state.interpolated_fields_cache = {}
            
            st.session_state.interpolated_fields_cache[cache_key] = {
                'values': interpolated_field,
                'metadata': self.last_interpolation_metadata,
                'timestamp': time.time()
            }
...
def main():
    ...
    if 'data_loader' not in st.session_state:
        ...
        st.session_state.interpolated_fields_cache = {}
        st.session_state.last_interpolation_key = None
...
def render_interpolation_extrapolation():
    ...
    # Run prediction - check if parameters have changed from last run
    current_params = {
        'energy': energy_query,
        'duration': duration_query,
        'max_time': max_time,
    ...
    # Check if we need to rerun predictions
    run_prediction = False
    if st.button("🚀 Run Physics-Informed Prediction", type="primary", use_container_width=True):
        run_prediction = True
    elif 'last_prediction_params' in st.session_state:
        # Compare current params with last run params
        last_params = st.session_state.last_prediction_params
        for k, v in current_params.items():
            if k not in last_params or last_params[k] != v:
                run_prediction = True
                break
    ...
    if run_prediction:
        with st.spinner("Running multi-head attention prediction with spatial locality regulation..."):
            ...
            if results and 'field_predictions' in results and results['field_predictions']:
                ...
                st.session_state.interpolated_fields_cache = {}  # Clear cache when parameters change
    ...
    # Only show visualization tabs if we have results
    if st.session_state.interpolation_results and st.session_state.interpolation_params:
        ...
        time_points = st.session_state.interpolation_params['time_points']
        energy_query = st.session_state.interpolation_params['energy_query']
        duration_query = st.session_state.interpolation_params['duration_query']
        ...
        with tab5:
            render_3d_interpolation(results, time_points, energy_query, duration_query,
                                   enable_3d, optimize_performance, 
                                   top_k if optimize_performance else None,
                                   subsample_factor if optimize_performance else 1)  # Pass subsample_factor
    else:
        st.info("Click 'Run Physics-Informed Prediction' to generate results for visualization.")
...
def render_3d_interpolation(results, time_points, energy_query, duration_query,
                           enable_3d=True, optimize_performance=False, top_k=10, subsample_factor=1):
    """Render 3D interpolation visualization with proper session state caching"""
    ...
    col1, col2 = st.columns(2)
    with col1:
        # Create a unique key for the field selection that persists
        selected_field_3d = st.selectbox(
            "Select Field for 3D Rendering",
            available_fields,
            index=0,
            key="interp_3d_field_selector",  # Fixed key for persistency
    ...
    with col2:
        timestep_idx_3d = st.slider(
            ...
            key="interp_3d_timestep_slider",  # Fixed key for persistency
    ...
    # Create a unique cache key based on current parameters
    cache_key_base = f"{selected_field_3d}_{energy_query}_{duration_query}_{selected_time_3d}_{subsample_factor}"
    ...
    original_weights = attention_weights_3d.copy()
    ...
    # Compute interpolated field with caching
    cache_key = f"{cache_key_base}_{hash(str(attention_weights_3d[:10]))}"
    
    # Check if we have this field already in session state cache
    interpolated_values = None
    if 'interpolated_fields_cache' in st.session_state and cache_key in st.session_state.interpolated_fields_cache:
        cached_data = st.session_state.interpolated_fields_cache[cache_key]
        # Verify the cached data is still valid
        if ('timestamp' in cached_data and 
            time.time() - cached_data['timestamp'] < 3600):  # Cache valid for 1 hour
            interpolated_values = cached_data['values']
            st.info(f"Using cached interpolation for {selected_field_3d} at t={selected_time_3d} ns")
    
    # If not in cache, compute the interpolation
    if interpolated_values is None:
        with st.spinner(f"Interpolating {selected_field_3d} field at t={selected_time_3d} ns..."):
            interpolated_values = st.session_state.extrapolator.interpolate_full_field(
                ...
                cache_key=cache_key,
                use_cache=True  # This will cache the result after computation
            )
        
        # If interpolation succeeded, it was already cached by the interpolate_full_field method
        if interpolated_values is None:
    ...
    first_sim = next(iter(st.session_state.simulations.values()))
    pts = first_sim['points'].copy()  # Make a copy to avoid modifying original
    triangles = first_sim.get('triangles')
    
    # Performance optimization: subsample points
    if optimize_performance and subsample_factor > 1 and pts.shape[0] > 1000:
