...
class UnifiedFEADataLoader:
    """Robust FEA data loader with lazy mesh loading and proper stats."""
    ...
    def load_all_simulations(_self, load_full_mesh: bool = True):
        simulations = {}
        summaries = []
        ...
        for folder_idx, folder in enumerate(folders):
            ...
            try:
                # Read first file to get mesh and field info
                mesh0 = meshio.read(vtu_files[0])
                if not mesh0.point_data:
                    st.warning(f"No point data in {name}")
                    continue

                points = mesh0.points.astype(np.float32) if load_full_mesh else None
                n_pts = len(mesh0.points)
                
                # Extract triangles if present
                triangles = None
                for cell_block in mesh0.cells:
                    if cell_block.type == "triangle":
                        triangles = cell_block.data.astype(np.int32) if load_full_mesh else None
                        break

                # Prepare fields arrays
                fields = {}
                field_info = {}
                ...
                for t in range(1, len(vtu_files)):
                    try:
                        ...
                        st.warning(f"⚠️ Error loading timestep {t} in {name}: {e}")

                # Build summary statistics (scalars, not lists)
                summary = {
                    'name': name,
                    'energy': energy,
                    'duration': duration,
                    'timesteps': len(vtu_files),
                    'field_stats': {}
                }
                
                for field, (kind, _) in field_info.items():
                    data = fields[field]
                    ...
                    if kind == "vector":
                        ...
                        mag = np.linalg.norm(data, axis=-1).flatten()
                        ...
                        if len(mag) > 0:
                            ...
                            summary['field_stats'][field] = {'min': 0.0, 'max': 0.0, 'mean': 0.0, 'std': 0.0}
                    else:
                        summary['field_stats'][field] = {
...
# =============================================
# ROBUST SUNBURST CHART (NO None COLORS)
# =============================================
def create_sunburst_chart(summaries, selected_field, colormap='Viridis', highlight_sim=None):
    """Create a sunburst chart with guaranteed colors and positive values."""
    labels = []
    parents = []
    values = []
    colors = []
    ...
    for e_key, e_sims in sorted(energy_groups.items(), key=lambda x: float(x[0].split()[0])):
        ...
        for s in e_sims:
            ...
            leaf_label = f"{selected_field}: {peak:.3f}"
            labels.append(leaf_label)
            parents.append(sim_label)
            # Ensure positive value (Plotly requirement)
            values.append(max(peak, 1e-6))
            # Leaf color will be set by colormap
            colors.append(None)   # temporary placeholder

    # Replace None placeholders with colormap colors
    leaf_indices = [i for i, c in enumerate(colors) if c is None]
    leaf_vals = [values[i] for i in leaf_indices]
    ...
    if leaf_vals:
        ...
        for idx in leaf_indices:
            norm_val = norm(values[idx])
            color_idx = min(int(norm_val * 100), 100)
            colors[idx] = color_scale[color_idx]
    
    # Final safety: any remaining None become a default color
    for i, c in enumerate(colors):
        if c is None:
            colors[i] = "#8c564b"   # brown fallback

    # Ensure all values are positive (extra safety)
    values = [max(v, 1e-6) for v in values]

    # Debug warnings (only in development, can be removed)
    if None in colors:
        st.warning("Sunburst: Some colors remained None after processing – using fallback.")
    if any(v <= 0 for v in values):
        st.warning("Sunburst: Non-positive values detected – forced to 1e-6.")
...
def main():
    ...
    with st.sidebar:
        ...
        load_full_data = st.checkbox("Load Full Mesh (for 3D)", value=True, 
                                     help="Uncheck to only load summary statistics (faster)")
        selected_colormap = st.selectbox("Default Colormap", EXTENDED_COLORMAPS, index=0, key="global_colormap")
        ...
        if st.session_state.data_loaded:
            ...
            if st.button("📥 Export Summaries (CSV)", use_container_width=True):
                df = pd.DataFrame([{
                    'name': s['name'],
                    'energy_mJ': s['energy'],
                    'duration_ns': s['duration'],
                    **{f"{fld}_max": stats['max'] for fld, stats in s['field_stats'].items()},
                    **{f"{fld}_mean": stats['mean'] for fld, stats in s['field_stats'].items()}
                } for s in st.session_state.summaries])
                csv = df.to_csv(index=False)
                st.download_button("📎 Download CSV", csv, "fea_summaries.csv", "text/csv", use_container_width=True)
    ...
    if not st.session_state.data_loaded:
        ...
        <div class="control-box" style="border-left-color: #ff9800; background: #fff3e0;">
            ...
            <p>Click <strong>🔄 Load Data</strong> in the sidebar to load simulations from the <code>fea_solutions</code> folder.</p>
        </div>
        """, unsafe_allow_html=True)
        return
...
def render_data_viewer(selected_colormap):
    st.markdown('<h2 class="sub-header">📁 3D Mesh Viewer</h2>', unsafe_allow_html=True)
    ...
    if not sim.get('has_mesh', False):
        st.warning("⚠️ This simulation was loaded without mesh data. To view 3D, reload with 'Load Full Mesh' enabled.")
        # Still show summary table and sunburst
    else:
        # ================= 3D VISUALIZATION (unchanged from original) =================
    ...
    # ================= PER-SIMULATION STATS TABLE =================
    with st.expander("📊 View all simulation statistics (min/max/mean/std)", expanded=False):
        if summaries:
            rows = []
            for s in summaries:
                row = {'Simulation': s['name'], 'Energy (mJ)': s['energy'], 'Duration (ns)': s['duration']}
                for fld, stats in s['field_stats'].items():
                    row[f"{fld}_max"] = stats['max']
                    row[f"{fld}_mean"] = stats['mean']
                rows.append(row)
    ...
    st.markdown("Hierarchy: **All Simulations → Energy → Simulation → Field Peak** (max over all timesteps)")

    if not summaries:
        st.warning("No summary data available for sunburst.")
        return

    # Get all fields from summaries
    all_fields = set()
    for s in summaries:
    ...
    with col1:
        default_field = 'temperature' if 'temperature' in available_fields else available_fields[0]
        field = st.selectbox("Select Field", available_fields, index=available_fields.index(default_field), key="sunburst_field")
    with col2:
        cmap = st.selectbox("Colormap", EXTENDED_COLORMAPS, 
                            ...
                            key="sunburst_cmap")
    with col3:
        highlight_sim = st.selectbox("Highlight Simulation", ["None"] + sorted(simulations.keys()), key="sunburst_highlight")
    ...
    st.markdown("### 🔁 Compare Two Fields")
    if len(available_fields) >= 2 and st.checkbox("Show two sunbursts side-by-side", key="show_two_sunbursts"):
        ...
        with colR:
            f2 = st.selectbox("Right Field", available_fields, index=min(1, len(available_fields)-1), key="sun_f2")
